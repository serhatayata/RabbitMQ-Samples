...
namespace RabbitMQProjects.ExcelCreateApp.Controllers
{
    ...
    public class ProductController : Controller
    {
        ...
        public ProductController(AppDbContext context, UserManager<IdentityUser> userManager, RabbitMQPublisher publisher)
        {
            _context = context;
        ...
        {
            var user = await _userManager.FindByNameAsync(User.Identity.Name);
            var files = await _context.UserFiles.Where(x => x.UserId == user.Id).OrderByDescending(t => t.CreatedDate).ToListAsync();
            return View(files);
        }
    }
}
